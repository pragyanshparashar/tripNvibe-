...
export default function FinalizeVotingControl({ tripOptions, leadingIndex, onFinalize, isSubmitting }) {
  ...
  return (
    <div className="rounded-3xl bg-dusk p-6 text-cream-50 shadow-soft">
      <p className="font-body text-xs font-semibold uppercase tracking-wide text-deep-200">
        Organizer action
      </p>
      <h3 className="mt-1 font-display text-lg font-bold">Ready to close voting?</h3>
      <p className="mt-1 font-body text-sm text-deep-100">
      ...
      <select
        ...
        className="mt-4 w-full rounded-xl border-2 border-white/20 bg-white/10 px-4 py-3 font-body text-sm text-cream-50 outline-none focus:border-coral-400"
      >
        {tripOptions.map((option, index) => (
          <option key={index} value={index} className="text-deep-900">
            {option.title}
          </option>
        ))}
      </select>
