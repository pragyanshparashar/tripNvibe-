        ...
        <div className="mt-14 grid gap-8 sm:grid-cols-2 lg:grid-cols-5">
          {steps.map((step, index) => (
            <div key={step.title} className="relative flex flex-col items-center text-center">
              <div className="glass-strong flex h-16 w-16 items-center justify-center rounded-2xl text-3xl">
                {step.emoji}
              </div>
              <span className="mt-4 font-display text-xs font-bold uppercase tracking-wider text-teal-500">
                Step {index + 1}
              </span>
              <h3 className="mt-1 font-display text-base font-semibold text-navy-900">{step.title}</h3>
              <p className="mt-2 font-body text-sm text-navy-500">{step.description}</p>
            </div>
          ))}
        </div>
      </div>
